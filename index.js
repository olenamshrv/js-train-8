...
function customUnshift(arr, ...elements) {
  ...
  if (Array.isArray(arr)) {
    ...
    for (let i = [...elements].length - 1; i >= 0; i--) {
      newLength = arr.unshift([...elements][i]);
    }
...
function customIncludes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    // let elemAmount = arr.filter((arrSmall) =>
    //   arrSmall.includes(element)
    // ).length;
    // console.log(elemAmount);
    // return elemAmount > 0 ? true : false;

    let filteredArray = arr.filter((arrSmall) => arrSmall.includes(element));
